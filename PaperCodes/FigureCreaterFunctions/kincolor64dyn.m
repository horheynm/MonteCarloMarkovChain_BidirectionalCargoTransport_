    ...
    for ii = 0:legend_num
        if ii >0
            if ii > 0 && ii <= ceil(legend_num/2)
                r = 0+(0.1);
                b = 0.5 + ii/legend_num*(0.9);
            else
                r = ((legend_num / ceil(legend_num/2) - legend_num / ii-0.1));
            end
            g = (1.5*ii/legend_num );
            if g > 1
                g = 0.9;
...
elseif kin.type == 2 && motType == 1 %  [0.1 0.5 0.1]
    name = 'kin2';
    for ii = 0:legend_num
        if ii >0
            if ii > 0 && ii <= ceil(legend_num/2)
                r = 0.095; b = r;
                g = 0.5 + ii/legend_num*(0.9);
            else
                g = 0.99 + 0.01;
                r =  + 0.73*(ii-ceil(legend_num/2)-1)/(legend_num - ceil(legend_num/2)-1) ;
                b = r;
            end
            if g > 1
                g = 1;
...
elseif motType == 2 
    ...
    for ii = 0:legend_num
        if ii >0
            r = (legend_num - (ii-1)) / (legend_num+1) + (1-((legend_num - (1-1)) / (legend_num+1)))*rand(1);
