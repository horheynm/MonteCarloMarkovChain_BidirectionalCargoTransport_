function [v,ek] = emptomatrix3(Fs,Fd,v,emp,ek)
...
% k = 1/average(time);
ek(Fs,Fd).r = zeros(2);
ek(Fs,Fd).r(1,1) = 1/ mean(v(Fs,Fd).kd(:,2)); % kin emp det
ek(Fs,Fd).r(2,1) = 1/ mean(v(Fs,Fd).ka(:,2)); % kin emp att
ek(Fs,Fd).r(1,2) = 1/ mean(v(Fs,Fd).dd(:,2)); % dyn emp det
ek(Fs,Fd).r(2,2) = 1/ mean(v(Fs,Fd).da(:,2)); % dyn emp att
kin = [ek(Fs,Fd).r(1,1);ek(Fs,Fd).r(2,1)];
dyn= [ek(Fs,Fd).r(1,2);ek(Fs,Fd).r(2,2)];
str = {'kdet';'kon'};
ek(Fs,Fd).table = table(kin,dyn,'RowNames',str);
