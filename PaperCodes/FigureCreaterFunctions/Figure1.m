...
set(gcf,'pos',[pos(1),pos(2),3.25,3.25*6/7]);



hold on;
h3 = plot(zeros(1,100),linspace(0,800,length(zeros(1,100))),'color','k','linewidth',1,'linestyle','--');

a1(1) = find(A == -6); a1(2)=find(A == 6); a2 = a1(2):a1(1);
h1 = plot(fliplr(-1.*A(a2,1)')',A(a2,2),'linew',1); % plot kinesin-1 FV from -6 to 6pN
...
a1(1) = find(A == -6); a1(2)=find(A == 6); a2 = a1(2):a1(1);
h2 = plot(fliplr(-1.*A(a2,1)')',A(a2,2),'linew',1); % plot kinesin-1 FV from -6 to 6pN
...
L.Box = 'off';
L.Position = [0.56028 0.39864 0.35684 0.1631];
L. FontSize = 8;
...
hold on;
h3 = plot(zeros(1,100),linspace(0,1200,length(zeros(1,100))),'color','k','linewidth',1,'LineStyle','--');

a1(1) = find(A == -6); a1(2)=find(A == 6); a2 = a1(2):a1(1); D2 = mean(D);
h1 = plot(fliplr(-1.*A(a2,1)')',(D2(a2)),'linew',1); % plot kinesin-1 FV from -6 to 6pN
...
a1(1) = find(A == -6); a1(2)=find(A == 6); a2 = a1(2):a1(1); D2 = mean(D);
h2 = plot(fliplr(-1.*A(a2,1)')',D2(a2),'linew',1); % plot kinesin-1 FV from -6 to 6pN
h2.MarkerFaceColor = 'w';h2.Color = [0.1 0.5 0.1];
...
L = legend([h1,h2],'Kinesin-1','Kinesin-2');
L.Box = 'off';
L.Position = [0.5934 0.52992 0.32051 0.1631];
L.FontSize = 8;
...
pathname1 = '/Volumes/Seagate/BiophysPaper2017_2/Figures/';
n = '//';
f = fullfile(pathname1,n);
if exist(f,'dir') == 0
    mkdir(pathname1,n)
end
nam = sprintf('Figure_1B');
baseFileName1 = sprintf(nam);
fullFileName1 = fullfile(pathname1,n,baseFileName1);
%
i=input('Enter "1" to save figure \n');
...
hold on;
h3 = plot(zeros(1,100),linspace(-1000,1200,length(zeros(1,100))),'color','k','linewidth',1,'lineStyle','--');
% h4 = plot(-10:1:10,zeros(length(-10:1:10)),'linew',2','color','k');

a1(1) = find(A == -8); a1(2)=find(A == 6); a2 = a1(2):a1(1); D2 = mean(D);
h1 = plot(A(:,1),A(:,2));
h1.MarkerFaceColor = 'w';h1.Color = 0.7*ones(1,3);  % Dyn 1.25 color
h1.MarkerSize = 10;hold on; axis([-6.3,8.3,-250, 100]); hold on
...
a1(1) = find(A == -8); a1(2)=find(A == 6); a2 = a1(2):a1(1); D2 = mean(D); hold on;
% h2 = plot(fliplr(-1.*A(a2,1)')',abs((A(a2,2))),'linew',1); % plot kinesin-1 FV from -6 to 6pN
h2 = plot(A(:,1),A(:,2));

h2.MarkerFaceColor = 'w';h2.Color = 'k';
...
L.Box = 'off';
L.Position = [0.5706 0.62708 0.3547 0.15119];
L. FontSize = 10;
...
a1(1) = find(A == -8); a1(2)=find(A == 6); a2 = a1(2):a1(1); D2 = mean(D);
h1 = plot(fliplr(-1.*A(a2,1)')', abs(D2 (fliplr(a2))),'linew',1); % plot dynein FV from -6 to 6pN
h1.MarkerFaceColor = 'w';h1.Color = 0.7*ones(1,3);  % Dyn 1.25 color
h1.MarkerSize = 10;hold on; axis([-6.3,8.3,0, 250]); hold on
...
a1(1) = find(A == -8); a1(2)=find(A == 6); a2 = a1(2):a1(1); D2 = mean(D); hold on;
h2 = plot(fliplr(-1.*A(a2,1)')',abs(D2(fliplr(a2))),'linew',1); % plot kinesin-1 FV from -6 to 6pN
h2.MarkerFaceColor = 'w';h2.Color = 'k';
h2.MarkerSize = 10;hold on; 
...
h1 = plot(fliplr(-1.*A(a2,1)')', (A(a2,2)),'linew',2); % plot dynein FV from -6 to 6pN
...
h2 = plot(fliplr(-1.*A(a2,1)')',(A(a2,2)),'linew',2); % plot kinesin-1 FV from -6 to 6pN
...
a1(1) = find(A == -6); a1(2)=find(A == 6); a2 = a1(1):a1(2); D2 = mean(D);
h1 = plot(fliplr(-1.*A(a2,1)')', abs(D2 (a2)),'linew',2); % plot dynein FV from -6 to 6pN
h1.MarkerFaceColor = 'w';h1.Color = [0.8 0.2 0.2];  % Dyn 1.25 color
...
a1(1) = find(A == -6); a1(2)=find(A == 6); a2 = a1(1):a1(2); D2 = mean(D); hold on;
h2 = plot(fliplr(-1.*A(a2,1)')',abs(D2(a2)),'linew',2); % plot kinesin-1 FV from -6 to 6pN
h2.MarkerFaceColor = 'w';h2.Color = [0.6 0.1 0.2];
h2.MarkerSize = 10;hold on; 
