...
dyn.step = 212/8; % [1/s], used for kon (only for mechanical activation)
...
% simulation category
category=1; % 1=simu run until all motor detach/run for 5s || 2=fixed time, last motor not allowed to detach, terminate at 5s
...
if save_mov == 1
%     movie2avi(mov,'my_movie.avi', 'compression', 'None');
    close(v)
